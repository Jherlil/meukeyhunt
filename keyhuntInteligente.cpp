...
std::atomic<uint64_t> checked_keys(0);

void *thread_ml_reporter(void *) {
  while (!GLOBAL_STOP.load()) {
    std::this_thread::sleep_for(std::chrono::seconds(30));
    printf("[IA] Atualização: chaves testadas: %" PRIu64 "\n",
           checked_keys.load());
...
const char *Ccoinbuffer_default =
...
int searchbinary(struct address_value *buffer, char *data,
...
int bsgs_searchbinary(struct bsgs_xvalue *arr, char *data, int64_t array_length,
                      uint64_t *r_value);
int bsgs_secondcheck(Int *start_range, uint32_t a, uint32_t k_index,
                     Int *privatekey);
int bsgs_thirdcheck(Int *start_range, uint32_t a, uint32_t k_index,
                    Int *privatekey);
...
const char *bsgs_modes[5] = {"sequential", "backward", "both", "random",
                             "dance"};
const char *modes[7] = {"xpoint",  "address",  "bsgs",  "rmd160",
                        "pub2rmd", "minikeys", "vanity"};
...
uint8_t ***vanity_rmd_limit_values_A = NULL,
        ***vanity_rmd_limit_values_B = NULL;
...
const char *str_limits_prefixs[7] = {"Mkeys/s", "Gkeys/s", "Tkeys/s", "Pkeys/s",
                                     "Ekeys/s", "Zkeys/s", "Ykeys/s"};
const char *str_limits[7] = {"1000000",
                             "1000000000",
                             "1000000000000",
                             "1000000000000000",
                             "1000000000000000000",
...
Point BSGS_P; // Original P is actually G, but this P value change over time for
              // calculations
...
int main(int argc, char **argv) {
  ...
  printf("====================================\n\n");
    ...
    // 1) Registra a função base58 -> sha256 **ANTES** de qualquer uso
    b58_sha256_impl = my_base58_to_sha256;

    // 2) Agora sim podemos inicializar a IA
    MLEngine::ml_init("models/model.pt", "models/positive_hits_features.csv");
    MLEngine::ml_load_training_data("models/positive_hits_features.csv", true);
    MLEngine::ml_load_training_data("models/negative_hits_features.csv", false);

  // ===== Variáveis de uso geral =====
  // ===== Variáveis de uso geral =====
  int c;                   // getopt
  int i = 0, j = 0, s = 0; // iteradores
  int index_value = -1;    // usado em getopt para modos

  /* --- controle de I/O --- */
  size_t rsize = 0;  // bytes lidos/escritos por fread/fwrite
  size_t readed = 0; // idem (mantido nome original)
                     // idem (mantido nome original)
  ...
  // Bloom filters
  uint64_t itemsbloom = 0;
  uint64_t itemsbloom2 = 0;
  uint64_t itemsbloom3 = 0;
  char *bf_ptr = NULL;

  // Threads
  bool finished = false;
  struct bPload *bPload_temp_ptr = NULL;
  char *bPload_threads_available = NULL;
  tothread *tt = NULL;

  // Inteiros usados (classe Int custom)
  Int int58;
  Int int_aux;
  Int int_r;
  Int int_q;
  ...
  while ((c = getopt(argc, argv,
                     "deh6MqRSB:b:c:C:E:f:I:k:l:m:N:n:p:r:s:t:v:G:8:z:")) !=
         -1) {
    switch (c) {
    case 'h':
      menu();
    ...
    case 'b':
      ...
      if (bitrange > 0 && bitrange <= 256) {
        ...
        checkpointer((void *)bit_range_str_min, __FILE__, "malloc",
                     "bit_range_str_min", __LINE__ - 1);
        MPZAUX.Set(&ONE);
        ...
        checkpointer((void *)bit_range_str_max, __FILE__, "malloc",
                     "bit_range_str_min", __LINE__ - 1);
        FLAGBITRANGE = 1;
      } else {
    ...
    case 'C':
      if (strlen(optarg) == 22) {
        ...
        checkpointer((void *)str_baseminikey, __FILE__, "malloc",
                     "str_baseminikey", __LINE__ - 1);
        raw_baseminikey = (char *)malloc(23);
        checkpointer((void *)raw_baseminikey, __FILE__, "malloc",
                     "raw_baseminikey", __LINE__ - 1);
        strncpy(str_baseminikey, optarg, 22);
        for (i = 0; i < 21; i++) {
          if (strchr(Ccoinbuffer, str_baseminikey[i + 1]) != NULL) {
            raw_baseminikey[i] =
                (int)(strchr(Ccoinbuffer, str_baseminikey[i + 1]) -
                      Ccoinbuffer) %
                58;
          } else {
    ...
    case 'M':
      ...
      break;
    case 'm':
      if (strcmp(optarg, "address") == 0) {
        FLAGMODE = MODE_ADDRESS;
        printf("[+] Mode address\n");
      } else if (strcmp(optarg, "rmd160") == 0) {
        FLAGMODE = MODE_RMD160;
        FLAGCRYPTO = CRYPTO_BTC;
        printf("[+] Mode rmd160\n");
      } else {
        fprintf(stderr,
                "[E] Unsupported mode: %s. Only 'address' and 'rmd160' are "
                "allowed.\n",
                optarg);
        exit(EXIT_FAILURE);
      }
    ...
    case 'v':
      ...
      if (vanity_bloom == NULL) {
        ...
        checkpointer((void *)vanity_bloom, __FILE__, "calloc", "vanity_bloom",
                     __LINE__ - 1);
      }
    ...
    case '8':
      if (strlen(optarg) == 58) {
        ...
        fprintf(stderr,
                "[E] The base58 alphabet must be 58 characters long.\n");
        exit(EXIT_FAILURE);
      }
      break;
    case 'z':
  ...
  if (FLAGMODE == MODE_ADDRESS &&
      FLAGCRYPTO == CRYPTO_NONE) { // When none crypto is defined the default
                                   // search is for Bitcoin
    FLAGCRYPTO = CRYPTO_BTC;
  ...
  if (FLAGRANGE) {
    ...
    if (n_range_start.IsEqual(&n_range_end) == false) {
      if (n_range_start.IsLower(&secp->order) &&
          n_range_end.IsLowerOrEqual(&secp->order)) {
        if (n_range_start.IsGreater(&n_range_end)) {
          fprintf(stderr, "[W] Opps, start range can't be great than end "
                          "range. Swapping them\n");
        ...
        fprintf(stderr, "[E] Start and End range can't be great than "
  ...
  if (FLAGMODE != MODE_BSGS) {
    if (FLAG_N) {
      ...
      if (N_SEQUENTIAL_MAX < 1024) {
        fprintf(stderr, "[I] n value need to be equal or great than 1024, back "
                        "to defaults\n");
        FLAG_N = 0;
    ...
    if (FLAGMODE == MODE_MINIKEYS) {
      ...
      checkpointer((void *)minikeyN, __FILE__, "malloc", "minikeyN",
                   __LINE__ - 1);
      i = 0;
      int58.SetInt32(58);
      int_aux.SetInt64(N_SEQUENTIAL_MAX);
      int_aux.Mult(253);
      /* We get approximately one valid mini key for each 256 candidates mini
       * keys since this is only statistics we multiply N_SEQUENTIAL_MAX by 253
       * to ensure not missed one one candidate minikey between threads... in
       * this approach we repeat from 1 to 3 candidates in each N_SEQUENTIAL_MAX
       * cycle IF YOU FOUND some other workaround please let me know */
      i = 20;
      salir = 0;
      do {
        if (!int_aux.IsZero()) {
...
if (FLAGMODE == MODE_BSGS) {
    ...
    /* -----------------------------------------------------------------
     * 1ª passada ─ contar quantas linhas (N) têm tamanho >= 66
     * ----------------------------------------------------------------- */
    ...
    while (fgets(aux, 1024, fd)) {
        ...
        if (len >= 66) ++N;                 /* 66 (compress) ou 130 (04...) */
    }
    ...
    /* -----------------------------------------------------------------
     * 2ª passada ─ voltar ao início e preencher estruturas
     * ----------------------------------------------------------------- */
    rewind(fd);
    ...
    while (fgets(aux, 1024, fd)) {
        ...
        switch (strlen(aux2)) {
        case 66:   /* chave comprimida 02/03 */
        case 130:  /* chave completa 04...  */
            if (secp->ParsePublicKeyHex(aux2,
                                        OriginalPointsBSGS[i],
                                        OriginalPointsBSGScompressed[i])) {
                ++i;
            } else {
                --N;   /* formato inválido → desconta */
            }
            break;
    ...
    if (bsgs_point_number > 0) {
        ...
        fprintf(stderr, "[E] The file doesn't have any valid public keys\n");
        exit(EXIT_FAILURE);
    }

    /* -----------------------------------------------------------------
     * Parâmetros padrão de BSGS (N-sqrt, M) — pode manter como estava
     * ----------------------------------------------------------------- */
    BSGS_N.SetInt32(0);
    BSGS_M.SetInt32(0);
...
M3	5497558139
  */
  ...
  bloom_bP_checksums =
      (struct checksumsha256 *)calloc(256, sizeof(struct checksumsha256));
  checkpointer((void *)bloom_bP_checksums, __FILE__, "calloc",
               "bloom_bP_checksums", __LINE__ - 1);
...
#endif
  checkpointer((void *)bloom_bP_mutex, __FILE__, "calloc", "bloom_bP_mutex",
               __LINE__ - 1);
  ...
  for (i = 0; i < 256; i++) {
    ...
    if (bloom_init2(&bloom_bP[i], itemsbloom, 0.000001) == 1) {
      fprintf(stderr, "[E] error bloom_init _ [%d]\n", i);
      exit(EXIT_FAILURE);
    }
    bloom_bP_totalbytes += bloom_bP[i].bytes;
    // if(FLAGDEBUG) bloom_print(&bloom_bP[i]);
  }
  printf(": %.2f MB\n", (float)((float)(uint64_t)bloom_bP_totalbytes /
                                (float)(uint64_t)1048576));
...
#endif
  checkpointer((void *)bloom_bPx2nd_mutex, __FILE__, "calloc",
               "bloom_bPx2nd_mutex", __LINE__ - 1);
  bloom_bPx2nd = (struct bloom *)calloc(256, sizeof(struct bloom));
  checkpointer((void *)bloom_bPx2nd, __FILE__, "calloc", "bloom_bPx2nd",
               __LINE__ - 1);
  bloom_bPx2nd_checksums =
      (struct checksumsha256 *)calloc(256, sizeof(struct checksumsha256));
  checkpointer((void *)bloom_bPx2nd_checksums, __FILE__, "calloc",
               "bloom_bPx2nd_checksums", __LINE__ - 1);
  ...
  printf(": %.2f MB\n", (float)((float)(uint64_t)bloom_bP2_totalbytes /
                                (float)(uint64_t)1048576));
...
#endif
  checkpointer((void *)bloom_bPx3rd_mutex, __FILE__, "calloc",
               "bloom_bPx3rd_mutex", __LINE__ - 1);
  bloom_bPx3rd = (struct bloom *)calloc(256, sizeof(struct bloom));
  checkpointer((void *)bloom_bPx3rd, __FILE__, "calloc", "bloom_bPx3rd",
               __LINE__ - 1);
  bloom_bPx3rd_checksums =
      (struct checksumsha256 *)calloc(256, sizeof(struct checksumsha256));
  checkpointer((void *)bloom_bPx3rd_checksums, __FILE__, "calloc",
               "bloom_bPx3rd_checksums", __LINE__ - 1);
  ...
  if (FLAGSAVEREADFILE) {
    // <--- declare aqui para uso nos fread
    ...
    if (fd_aux1 != NULL) {
      ...
      for (i = 0; i < 256; i++) {
        bf_ptr =
            (char *)bloom_bP[i].bf; /*We need to save the current bf pointer*/
        ...
        readed = fread(&bloom_bP_checksums[i], sizeof(struct checksumsha256), 1,
                       fd_aux1);
        if (readed != 1) {
          fprintf(stderr, "[E] Error reading the file %s\n", buffer_bloom_file);
          exit(EXIT_FAILURE);
        ...
        if (FLAGSKIPCHECKSUM == 0) {
          ...
          if (memcmp(bloom_bP_checksums[i].data, rawvalue, 32) != 0 ||
              memcmp(bloom_bP_checksums[i].backup, rawvalue, 32) !=
                  0) { /* Verification */
      ...
      snprintf(buffer_bloom_file, 1024, "keyhunt_bsgs_3_%" PRIu64 ".blm",
               bsgs_m);
      fd_aux1 = fopen(buffer_bloom_file, "rb");
      if (fd_aux1 != NULL) {
      ...
      if (fd_aux1 != NULL) {
        ...
        for (i = 0; i < 256; i++) {
          ...
          */
          ...
          memcpy(
              &bloom_bP[i], &oldbloom_bP,
              sizeof(
                  struct bloom)); // We only need to copy the part data to the
                                  // new bloom size, not from the old size
          bloom_bP[i].bf = (uint8_t *)bf_ptr; /* Restoring the bf pointer*/
        ...
        fclose(fd_aux1);
        FLAGUPDATEFILE1 =
            1; /* Flag to migrate the data to the new File keyhunt_bsgs_4_ */
        FLAGREADEDFILE1 = 1;
    ...
    snprintf(buffer_bloom_file, 1024, "keyhunt_bsgs_6_%" PRIu64 ".blm",
             bsgs_m2);
    ...
    if (fd_aux2 != NULL) {
      ...
      fflush(stdout);
      for (i = 0; i < 256; i++) {
        bf_ptr = (char *)bloom_bPx2nd[i]
                     .bf; /*We need to save the current bf pointer*/
        ...
        readed = fread(&bloom_bPx2nd_checksums[i],
                       sizeof(struct checksumsha256), 1, fd_aux2);
        if (readed != 1) {
          fprintf(stderr, "[E] Error reading the file %s\n", buffer_bloom_file);
          exit(EXIT_FAILURE);
        }
        ...
        if (FLAGSKIPCHECKSUM == 0) {
          ...
          if (memcmp(bloom_bPx2nd_checksums[i].data, rawvalue, 32) != 0 ||
              memcmp(bloom_bPx2nd_checksums[i].backup, rawvalue, 32) !=
                  0) { /* Verification */
            fprintf(stderr, "[E] Error checksum file mismatch! %s\n",
                    buffer_bloom_file);
            exit(EXIT_FAILURE);
      ...
      printf(" Done!\n");
      memset(buffer_bloom_file, 0, 1024);
      snprintf(buffer_bloom_file, 1024, "keyhunt_bsgs_5_%" PRIu64 ".blm",
               bsgs_m2);
      fd_aux2 = fopen(buffer_bloom_file, "rb");
      if (fd_aux2 != NULL) {
        printf("[W] Unused file detected %s you can delete it without worry\n",
      ...
      memset(buffer_bloom_file, 0, 1024);
      snprintf(buffer_bloom_file, 1024, "keyhunt_bsgs_1_%" PRIu64 ".blm",
               bsgs_m2);
      fd_aux2 = fopen(buffer_bloom_file, "rb");
      if (fd_aux2 != NULL) {
        printf("[W] Unused file detected %s you can delete it without worry\n",
    ...
    snprintf(buffer_bloom_file, 1024, "keyhunt_bsgs_2_%" PRIu64 ".tbl",
             bsgs_m3);
    ...
    snprintf(buffer_bloom_file, 1024, "keyhunt_bsgs_7_%" PRIu64 ".blm",
             bsgs_m3);
    fd_aux2 = fopen(buffer_bloom_file, "rb");
    if (fd_aux2 != NULL) {
      printf("[+] Reading bloom filter from file %s ", buffer_bloom_file);
      ...
      for (i = 0; i < 256; i++) {
        ...
        readed = fread(&bloom_bPx3rd_checksums[i],
                       sizeof(struct checksumsha256), 1, fd_aux2);
        if (readed != 1) {
          fprintf(stderr, "[E] Error reading the file %s\n", buffer_bloom_file);
          exit(EXIT_FAILURE);
        ...
        if (FLAGSKIPCHECKSUM == 0) {
          ...
          if (memcmp(bloom_bPx3rd_checksums[i].data, rawvalue, 32) != 0 ||
              memcmp(bloom_bPx3rd_checksums[i].backup, rawvalue, 32) !=
                  0) { /* Verification */
            fprintf(stderr, "[E] Error checksum file mismatch! %s\n",
                    buffer_bloom_file);
            exit(EXIT_FAILURE);
  ...
  if (!FLAGREADEDFILE1 || !FLAGREADEDFILE2 || !FLAGREADEDFILE3 ||
      !FLAGREADEDFILE4) {
    if (FLAGREADEDFILE1 == 1) {
      /*
              ...
              - bp Table 0.25 %
      */
      printf("[I] We need to recalculate some files, don't worry this is only "
             "3%% of the previous work\n");
      ...
      printf("\r[+] processing %lu/%lu bP points : %i%%\r", FINISHED_ITEMS,
             bsgs_m, (int)(((double)FINISHED_ITEMS / (double)bsgs_m) * 100));
      fflush(stdout);
      ...
      bPload_mutex =
          (pthread_mutex_t *)calloc(NTHREADS, sizeof(pthread_mutex_t));
#endif
      checkpointer((void *)bPload_mutex, __FILE__, "calloc", "bPload_mutex",
                   __LINE__ - 1);
      bPload_temp_ptr =
          (struct bPload *)calloc(NTHREADS, sizeof(struct bPload));
      checkpointer((void *)bPload_temp_ptr, __FILE__, "calloc",
                   "bPload_temp_ptr", __LINE__ - 1);
      bPload_threads_available = (char *)calloc(NTHREADS, sizeof(char));
      checkpointer((void *)bPload_threads_available, __FILE__, "calloc",
                   "bPload_threads_available", __LINE__ - 1);
      ...
      do {
...
#if defined(_WIN64) && !defined(__CYGWIN__)
            tid[j] = CreateThread(NULL, 0, thread_bPload_2blooms,
                                  (void *)&bPload_temp_ptr[j], 0, &s);
#else
            s = pthread_create(&tid[j], NULL, thread_bPload_2blooms,
        ...
        if (OLDFINISHED_ITEMS != FINISHED_ITEMS) {
          printf("\r[+] processing %lu/%lu bP points : %i%%\r", FINISHED_ITEMS,
                 bsgs_m2,
                 (int)(((double)FINISHED_ITEMS / (double)bsgs_m2) * 100));
          fflush(stdout);
      ...
      do {
        for (j = 0; j < NTHREADS && !salir; j++) {
          ...
          if (bPload_threads_available[j] && !salir) {
            ...
            // if(FLAGDEBUG) printf("[I] %lu to
            // %lu\n",bPload_temp_ptr[i].from,bPload_temp_ptr[i].to);
#if defined(_WIN64) && !defined(__CYGWIN__)
            tid[j] = CreateThread(NULL, 0, thread_bPload,
                                  (void *)&bPload_temp_ptr[j], 0, &s);
#else
            s = pthread_create(&tid[j], NULL, thread_bPload,
                               (void *)&bPload_temp_ptr[j]);
            pthread_detach(tid[j]);
#endif
            BASE += THREADBPWORKLOAD;
        ...
        if (OLDFINISHED_ITEMS != FINISHED_ITEMS) {
          printf("\r[+] processing %lu/%lu bP points : %i%%\r", FINISHED_ITEMS,
                 bsgs_m,
                 (int)(((double)FINISHED_ITEMS / (double)bsgs_m) * 100));
          fflush(stdout);
          OLDFINISHED_ITEMS = FINISHED_ITEMS;
        }
  ...
  if (!FLAGREADEDFILE2) {
    for (i = 0; i < 256; i++) {
      ...
      memcpy(bloom_bPx2nd_checksums[i].backup, bloom_bPx2nd_checksums[i].data,
             32);
    }
    printf(".");
  }
  if (!FLAGREADEDFILE4) {
    for (i = 0; i < 256; i++) {
      ...
      memcpy(bloom_bPx3rd_checksums[i].backup, bloom_bPx3rd_checksums[i].data,
             32);
    }
    printf(".");
  }
  ...
  if (FLAGSAVEREADFILE || FLAGUPDATEFILE1) {
    if (!FLAGREADEDFILE1 || FLAGUPDATEFILE1) {
      snprintf(buffer_bloom_file, 1024, "keyhunt_bsgs_4_%" PRIu64 ".blm",
               bsgs_m);
      ...
      if (fd_aux1 != NULL) {
        ...
        for (i = 0; i < 256; i++) {
          ...
          readed = fwrite(&bloom_bP_checksums[i], sizeof(struct checksumsha256),
                          1, fd_aux1);
          if (readed != 1) {
            fprintf(stderr, "[E] Error writing the file %s please delete it\n",
                    buffer_bloom_file);
    ...
    if (!FLAGREADEDFILE2) {
      ...
      if (fd_aux2 != NULL) {
        ...
        for (i = 0; i < 256; i++) {
          ...
          readed =
              fwrite(bloom_bPx2nd[i].bf, bloom_bPx2nd[i].bytes, 1, fd_aux2);
          if (readed != 1) {
            fprintf(stderr, "[E] Error writing the file %s\n",
                    buffer_bloom_file);
            exit(EXIT_FAILURE);
          }
          readed = fwrite(&bloom_bPx2nd_checksums[i],
                          sizeof(struct checksumsha256), 1, fd_aux2);
          if (readed != 1) {
            fprintf(stderr, "[E] Error writing the file %s please delete it\n",
                    buffer_bloom_file);
      ...
      if (!FLAGREADEDFILE3) {
        ...
        if (fd_aux3 != NULL) {
          ...
          fflush(stdout);
          rsize = fwrite(bPtable, bytes, 1, fd_aux3);
          if (rsize != 1) {
            fprintf(stderr, "[E] Error writing the file %s\n",
                    buffer_bloom_file);
            exit(EXIT_FAILURE);
          }
          rsize = fwrite(checksum, 32, 1, fd_aux3);
          if (rsize != 1) {
            fprintf(stderr, "[E] Error writing the file %s\n",
                    buffer_bloom_file);
            exit(EXIT_FAILURE);
      ...
      if (!FLAGREADEDFILE4) {
        ...
        if (fd_aux2 != NULL) {
          ...
          for (i = 0; i < 256; i++) {
            ...
            readed = fwrite(&bloom_bPx3rd_checksums[i],
                            sizeof(struct checksumsha256), 1, fd_aux2);
            if (readed != 1) {
              fprintf(stderr,
                      "[E] Error writing the file %s please delete it\n",
                      buffer_bloom_file);
              exit(EXIT_FAILURE);
            }
            if (i % 64 == 0) {
      ...
      for (j = 0; j < NTHREADS; j++) {
        ...
        case 0:
          tid[j] =
              CreateThread(NULL, 0, thread_process_bsgs, (void *)tt, 0, &s);
          break;
        case 1:
          tid[j] = CreateThread(NULL, 0, thread_process_bsgs_backward,
                                (void *)tt, 0, &s);
          break;
        case 2:
          tid[j] = CreateThread(NULL, 0, thread_process_bsgs_both, (void *)tt,
                                0, &s);
          break;
        case 3:
          tid[j] = CreateThread(NULL, 0, thread_process_bsgs_random, (void *)tt,
                                0, &s);
          break;
        case 4:
          tid[j] = CreateThread(NULL, 0, thread_process_bsgs_dance, (void *)tt,
                                0, &s);
          break;
        }
#else
        case 0:
          ...
          break;
        case 1:
          s = pthread_create(&tid[j], NULL, thread_process_bsgs_backward,
                             (void *)tt);
          break;
        case 2:
          s = pthread_create(&tid[j], NULL, thread_process_bsgs_both,
                             (void *)tt);
          break;
        case 3:
          s = pthread_create(&tid[j], NULL, thread_process_bsgs_random,
                             (void *)tt);
          break;
        case 4:
          s = pthread_create(&tid[j], NULL, thread_process_bsgs_dance,
                             (void *)tt);
          break;
#endif
      }
  ...
  if (FLAGMODE != MODE_BSGS) {
    ...
    for (j = 0; j < NTHREADS; j++) {
      ...
      case MODE_MINIKEYS:
        tid[j] =
            CreateThread(NULL, 0, thread_process_minikeys, (void *)tt, 0, &s);
        break;
      case MODE_VANITY:
        tid[j] =
            CreateThread(NULL, 0, thread_process_vanity, (void *)tt, 0, &s);
        break;
#else
        case MODE_ADDRESS:
        case MODE_XPOINT:
        case MODE_RMD160:
          ...
          break;
        case MODE_MINIKEYS:
          s = pthread_create(&tid[j], NULL, thread_process_minikeys,
                             (void *)tt);
          break;
        case MODE_VANITY:
      ...
      if (MPZAUX.IsZero()) {
        ...
        if (pretotal.IsLower(&int_limits[0])) {
          if (FLAGMATRIX) {
            ...
            sprintf(buffer, "\r[+] Total %s keys in %s seconds: %s keys/s\r",
                    str_total, str_seconds, str_pretotal);
          }
        } else {
          i = 0;
...
char *pubkeytopubaddress(char *pkey, int length) {
  ...
  checkpointer((void *)pubaddress, __FILE__, "malloc", "pubaddress",
               __LINE__ - 1);
...
DWORD WINAPI thread_process_minikeys(LPVOID vargp) {
  ...
  char address[4][40], minikey[4][24], minikeys[8][24], buffer_b58[21],
      minikey2check[24], rawvalue[4][32];
      ...
      do {
        for (j = 0; j < 256; j++) {
          ...
          do {
            ...
            sha256sse_23((uint8_t *)minikey[0], (uint8_t *)minikey[1],
                         (uint8_t *)minikey[2], (uint8_t *)minikey[3],
                         (uint8_t *)rawvalue[0], (uint8_t *)rawvalue[1],
                         (uint8_t *)rawvalue[2], (uint8_t *)rawvalue[3]);
            for (k = 0; k < 4; k++) {
          ...
          count_valid -= 4;
          sha256sse_22((uint8_t *)minikeys[0], (uint8_t *)minikeys[1],
                       (uint8_t *)minikeys[2], (uint8_t *)minikeys[3],
                       (uint8_t *)rawvalue[0], (uint8_t *)rawvalue[1],
                       (uint8_t *)rawvalue[2], (uint8_t *)rawvalue[3]);
          ...
          secp->GetHash160(P2PKH, false, publickey[0], publickey[1],
          ...
          for (k = 0; k < 4; k++) {
            ...
            if (r) {
              r = searchbinary(addressTable, publickeyhashrmd160_uncompress[k],
                               N);
              if (r) {
                /* hit */
                hextemp = key_mpz[k].GetBase16();
                secp->GetPublicKeyHex(false, publickey[k],
                ...
                rmd160toaddress_dst(publickeyhashrmd160_uncompress[k],
                                    address[k]);
                minikeys[k][22] = '\0';
                if (keys != NULL) {
                  fprintf(
                      keys,
                      "Private Key: %s\npubkey: %s\nminikey: %s\naddress: %s\n",
                      hextemp, public_key_uncompressed_hex, minikeys[k],
                      address[k]);
                  fclose(keys);
                }
                printf("\nHIT!! Private Key: %s\npubkey: %s\nminikey: "
...
DWORD WINAPI thread_process(LPVOID vargp) {
  ...
  bool calculate_y = FLAGSEARCH == SEARCH_UNCOMPRESS ||
                     FLAGSEARCH == SEARCH_BOTH || FLAGCRYPTO == CRYPTO_ETH;
  ...
  do {
    ...
    if (continue_flag) {
      ...
      do {
        ...
        /*
                Half point for endomorphism because pts[CPU_GRP_SIZE / 2] was
           not calcualte in the previous cycle
        */
        if (FLAGENDOMORPHISM) {
          if (calculate_y) {

            endomorphism_beta[CPU_GRP_SIZE / 2].y.Set(&pts[CPU_GRP_SIZE / 2].y);
            endomorphism_beta2[CPU_GRP_SIZE / 2].y.Set(
                &pts[CPU_GRP_SIZE / 2].y);
          }
          endomorphism_beta[CPU_GRP_SIZE / 2].x.ModMulK1(
              &pts[CPU_GRP_SIZE / 2].x, &beta);
          endomorphism_beta2[CPU_GRP_SIZE / 2].x.ModMulK1(
              &pts[CPU_GRP_SIZE / 2].x, &beta2);
        }
        ...
        /*
                First point for endomorphism because pts[0] was not calcualte
           previously
        */
        if (FLAGENDOMORPHISM) {
          if (calculate_y) {
          ...
          case MODE_ADDRESS:
            if (FLAGCRYPTO == CRYPTO_BTC) {
              ...
              if (FLAGSEARCH == SEARCH_COMPRESS || FLAGSEARCH == SEARCH_BOTH) {
                if (FLAGENDOMORPHISM) {
                  secp->GetHash160_fromX(
                      P2PKH, 0x02, &pts[(j * 4)].x, &pts[(j * 4) + 1].x,
                      &pts[(j * 4) + 2].x, &pts[(j * 4) + 3].x,
                      (uint8_t *)publickeyhashrmd160_endomorphism[0][0],
                  ...
                  secp->GetHash160_fromX(
                      P2PKH, 0x03, &pts[(j * 4)].x, &pts[(j * 4) + 1].x,
                      &pts[(j * 4) + 2].x, &pts[(j * 4) + 3].x,
                      (uint8_t *)publickeyhashrmd160_endomorphism[1][0],
                  ...
                  secp->GetHash160_fromX(
                      P2PKH, 0x02, &endomorphism_beta[(j * 4)].x,
                      &endomorphism_beta[(j * 4) + 1].x,
                      &endomorphism_beta[(j * 4) + 2].x,
                      &endomorphism_beta[(j * 4) + 3].x,
                  ...
                  secp->GetHash160_fromX(
                      P2PKH, 0x03, &endomorphism_beta[(j * 4)].x,
                      &endomorphism_beta[(j * 4) + 1].x,
                      &endomorphism_beta[(j * 4) + 2].x,
                      &endomorphism_beta[(j * 4) + 3].x,
                  ...
                  secp->GetHash160_fromX(
                      P2PKH, 0x02, &endomorphism_beta2[(j * 4)].x,
                      &endomorphism_beta2[(j * 4) + 1].x,
                      &endomorphism_beta2[(j * 4) + 2].x,
                      &endomorphism_beta2[(j * 4) + 3].x,
                  ...
                  secp->GetHash160_fromX(
                      P2PKH, 0x03, &endomorphism_beta2[(j * 4)].x,
                      &endomorphism_beta2[(j * 4) + 1].x,
                      &endomorphism_beta2[(j * 4) + 2].x,
                      &endomorphism_beta2[(j * 4) + 3].x,
              ...
              if (FLAGSEARCH == SEARCH_UNCOMPRESS ||
                ...
                if (FLAGENDOMORPHISM) {
                  ...
                  secp->GetHash160(
                      P2PKH, false, endomorphism_negeted_point[0],
                      endomorphism_negeted_point[1],
                      endomorphism_negeted_point[2],
                      endomorphism_negeted_point[3],
                  ...
                  secp->GetHash160(
                      P2PKH, false, endomorphism_beta[(j * 4)],
                      endomorphism_beta[(j * 4) + 1],
                      endomorphism_beta[(j * 4) + 2],
                      endomorphism_beta[(j * 4) + 3],
                  ...
                  for (l = 0; l < 4; l++) {
                    endomorphism_negeted_point[l] =
                        secp->Negation(endomorphism_beta2[(j * 4) + l]);
                  }
                  secp->GetHash160(
                      P2PKH, false, endomorphism_beta2[(j * 4)],
                      endomorphism_beta2[(j * 4) + 1],
                      endomorphism_beta2[(j * 4) + 2],
                      endomorphism_beta2[(j * 4) + 3],
              ...
              if (FLAGENDOMORPHISM) {
                for (k = 0; k < 4; k++) {
                  endomorphism_negeted_point[k] =
                      secp->Negation(pts[(j * 4) + k]);
                  generate_binaddress_eth(
                      pts[(4 * j) + k],
                  ...
                  endomorphism_negeted_point[k] =
                      secp->Negation(endomorphism_beta[(j * 4) + k]);
                  generate_binaddress_eth(
                      endomorphism_beta[(4 * j) + k],
                      (uint8_t *)publickeyhashrmd160_endomorphism[2][k]);
                  generate_binaddress_eth(
                      endomorphism_negeted_point[k],
                      (uint8_t *)publickeyhashrmd160_endomorphism[3][k]);
                  endomorphism_negeted_point[k] =
                      secp->Negation(endomorphism_beta2[(j * 4) + k]);
                  generate_binaddress_eth(
                      endomorphism_beta[(4 * j) + k],
              ...
              for (k = 0; k < 4; k++) {
                if (FLAGSEARCH == SEARCH_COMPRESS ||
                  ...
                  if (FLAGENDOMORPHISM) {
                    for (l = 0; l < 6; l++) {
                      r = bloom_check(&bloom,
                                      publickeyhashrmd160_endomorphism[l][k],
                                      MAXLENGTHADDRESS);
                      if (r) {
                        r = searchbinary(addressTable,
                                         publickeyhashrmd160_endomorphism[l][k],
                                         N);
                        if (r) {
                          keyfound.SetInt32(k);
                          ...
                          case 0: // Original point, prefix 02
                            if (publickey.y
                                    .IsOdd()) { // if the current publickey is
                                                // odd that means, we need to
                                                // negate the keyfound to get
                                                // the correct key
                              keyfound.Neg();
                              keyfound.Add(&secp->order);
                            }
                            // else we dont need to chage the current keyfound
                            // because it already have prefix 02
                            break;
                          case 1: // Original point, prefix 03
                            if (publickey.y
                                    .IsEven()) { // if the current publickey is
                                                 // even that means, we need to
                                                 // negate the keyfound to get
                                                 // the correct key
                              keyfound.Neg();
                              keyfound.Add(&secp->order);
                            ...
                            // else we dont need to chage the current keyfound
                            // because it already have prefix 03
                            break;
                    ...
                    for (l = 0; l < 2; l++) {
                      ...
                      if (r) {
                        ...
                        if (r) {
                          ...
                          publickey = secp->ComputePublicKey(&keyfound);
                          secp->GetHash160(P2PKH, true, publickey,
                                           (uint8_t *)publickeyhashrmd160);
                          if (memcmp(publickeyhashrmd160_endomorphism[l][k],
                                     publickeyhashrmd160, 20) != 0) {
                            keyfound.Neg();
                            keyfound.Add(&secp->order);
                          }
                          writekey(true, &keyfound);
                ...
                if (FLAGSEARCH == SEARCH_UNCOMPRESS ||
                  ...
                  if (FLAGENDOMORPHISM) {
                    for (l = 6; l < 12;
                         l++) { // We check the array from 6 to 12(excluded)
                                // because we save the uncompressed information
                                // there
                      r = bloom_check(
                          &bloom, publickeyhashrmd160_endomorphism[l][k],
                          MAXLENGTHADDRESS); // Check in Bloom filter
                      if (r) {
                        r = searchbinary(
                            addressTable,
                            publickeyhashrmd160_endomorphism[l][k],
                            N); // Check in Array using Binary search
                        if (r) {
                          keyfound.SetInt32(k);
                          keyfound.Mult(&stride);
                          keyfound.Add(&key_mpz);
                          ...
                          case 7:
                            publickey = secp->ComputePublicKey(&keyfound);
                            secp->GetHash160(
                                P2PKH, false, publickey,
                                (uint8_t *)publickeyhashrmd160_uncompress[0]);
                            if (memcmp(publickeyhashrmd160_endomorphism[l][k],
                                       publickeyhashrmd160_uncompress[0],
                                       20) != 0) {
                              keyfound.Neg();
                              keyfound.Add(&secp->order);
                            }
                    ...
                    r = bloom_check(&bloom, publickeyhashrmd160_uncompress[k],
                                    MAXLENGTHADDRESS);
                    if (r) {
                      r = searchbinary(addressTable,
                                       publickeyhashrmd160_uncompress[k], N);
                      if (r) {
                        keyfound.SetInt32(k);
                        keyfound.Mult(&stride);
                        keyfound.Add(&key_mpz);
          ...
          case MODE_XPOINT:
            for (k = 0; k < 4; k++) {
              if (FLAGENDOMORPHISM) {
                ...
                endomorphism_beta[(j * 4) + k].x.Get32Bytes(
                ...
                endomorphism_beta2[(j * 4) + k].x.Get32Bytes(
                    (unsigned char *)rawvalue);
                r = bloom_check(&bloom, rawvalue, MAXLENGTHADDRESS);
                if (r) {
                  r = searchbinary(addressTable, rawvalue, N);
...
DWORD WINAPI thread_process_vanity(LPVOID vargp) {
  ...
  // if FLAGENDOMORPHISM  == 1 and only compress search is enabled then there is
  // no need to calculate the Y value value

  bool calculate_y =
      FLAGSEARCH == SEARCH_UNCOMPRESS || FLAGSEARCH == SEARCH_BOTH;
  ...
  if(FLAGDEBUG && thread_number == 0)	{
          ...
          for(i =0; i < vanity_rmd_targets;i++)	{
                  printf("[D] vanity_rmd_limits[%li] =
  %i\n",i,vanity_rmd_limits[i]);

          }
          printf("[D] vanity_rmd_minimun_bytes_check_length =
        ...
        for (j = 0; j < CPU_GRP_SIZE / 4; j++) {
          ...
          if (FLAGSEARCH == SEARCH_UNCOMPRESS || FLAGSEARCH == SEARCH_BOTH) {
            if (FLAGENDOMORPHISM) {
              ...
              secp->GetHash160(
                  P2PKH, false, endomorphism_negeted_point[0],
                  endomorphism_negeted_point[1], endomorphism_negeted_point[2],
                  endomorphism_negeted_point[3],
                  (uint8_t *)publickeyhashrmd160_endomorphism[7][0],
                  (uint8_t *)publickeyhashrmd160_endomorphism[7][1],
                  (uint8_t *)publickeyhashrmd160_endomorphism[7][2],
              ...
              secp->GetHash160(P2PKH, false, pts[(j * 4)], pts[(j * 4) + 1],
                               pts[(j * 4) + 2], pts[(j * 4) + 3],
                               (uint8_t *)publickeyhashrmd160_uncompress[0],
                               (uint8_t *)publickeyhashrmd160_uncompress[1],
                               (uint8_t *)publickeyhashrmd160_uncompress[2],
          ...
          for (k = 0; k < 4; k++) {
            if (FLAGSEARCH == SEARCH_COMPRESS || FLAGSEARCH == SEARCH_BOTH) {
              if (FLAGENDOMORPHISM) {
                for (l = 0; l < 6; l++) {
                  if (vanityrmdmatch(
                          (uint8_t *)publickeyhashrmd160_endomorphism[l][k])) {
                    // Here the given publickeyhashrmd160 match againts one of
                    // the vanity targets We need to check which of the cases is
                    // it.
                    ...
                    case 0: // Original point, prefix 02
                      if (publickey.y
                              .IsOdd()) { // if the current publickey is odd
                                          // that means, we need to negate the
                                          // keyfound to get the correct key
                        keyfound.Neg();
                        keyfound.Add(&secp->order);
                      }
                      // else we dont need to chage the current keyfound because
                      // it already have prefix 02
                      break;
                    case 1: // Original point, prefix 03
                      if (publickey.y
                              .IsEven()) { // if the current publickey is even
                                           // that means, we need to negate the
                                           // keyfound to get the correct key
                        keyfound.Neg();
                        keyfound.Add(&secp->order);
                  ...
                  if (vanityrmdmatch(
                    ...
                    case 7:
                      ...
                      secp->GetHash160(
                          ...
                          (uint8_t *)publickeyhashrmd160_uncompress[0]);
                      if (memcmp(publickeyhashrmd160_endomorphism[l][k],
                                 publickeyhashrmd160_uncompress[0], 20) != 0) {
                        keyfound.Neg();
                        keyfound.Add(&secp->order);
                      }
                      break;
...
int64_t bsgs_partition(struct bsgs_xvalue *arr, int64_t n) {
  ...
  do {
    while (left < right &&
           memcmp(arr[left].value, pivot.value, BSGS_XVALUE_RAM) <= 0) {
      left++;
    }
    while (right >= left &&
           memcmp(arr[right].value, pivot.value, BSGS_XVALUE_RAM) > 0) {
      right--;
    }
    if (left < right) {
      if (left == r || right == r) {
...
int bsgs_searchbinary(struct bsgs_xvalue *buffer, char *data,
                      int64_t array_length, uint64_t *r_value) {
...
DWORD WINAPI thread_process_bsgs(LPVOID vargp) {
  ...
  do {
    /*
       ...
       threads so BSGS_CURRENT is never the same between threads
    */
#if defined(_WIN64) && !defined(__CYGWIN__)
    WaitForSingleObject(bsgs_thread, INFINITE);
#else
    ...
    base_key.Set(&BSGS_CURRENT); /* we need to set our base_key to the current
                                    BSGS_CURRENT value*/
    ...
    for (k = 0; k < bsgs_point_number; k++) {
      if (bsgs_found[k] == 0) {
        ...
        while (j < cycles && bsgs_found[k] == 0) {
          ...
          same inverse We compute key in the positive and negative way from the
          center of the group
          */
          // center point
          pts[CPU_GRP_SIZE / 2] = startP;
...
pn.y.ModAdd(&GSn[i].y);
          ...
          for (int i = 0; i < CPU_GRP_SIZE && bsgs_found[k] == 0; i++) {
            ...
            r = bloom_check(&bloom_bP[((unsigned char)xpoint_raw[0])],
                            xpoint_raw, 32);
            if (r) {
              ...
              if (r) {
                ...
                aux_c = secp->GetPublicKeyHex(OriginalPointsBSGScompressed[k],
                                              point_found);
                printf("[+] Publickey %s\n", aux_c);
#if defined(_WIN64) && !defined(__CYGWIN__)
                WaitForSingleObject(write_keys, INFINITE);
...
/*
        The bsgs_secondcheck function is made to perform a second BSGS search in
   a Range of less size. This funtion is made with the especific purpouse to USE
   a smaller bPtable in RAM.
*/
int bsgs_secondcheck(Int *start_range, uint32_t a, uint32_t k_index,
                     Int *privatekey) {
  int i = 0, found = 0, r = 0;
...
int bsgs_thirdcheck(Int *start_range, uint32_t a, uint32_t k_index,
  ...
  do {
    ...
    if (r) {
      ...
      /*
              For some reason the AddDirect don't return 000000... value when
         the publickeys are the negated values from each other Why JLP? This is
         is an special case
      */
...
DWORD WINAPI thread_bPload(LPVOID vargp) {
  ...
  for (uint64_t s = 0; s < nbStep; s++) {
    ...
    // We use the fact that P + i*G and P - i*G has the same deltax, so the same
    // inverse We compute key in the positive and negative way from the center
    // of the group center point
      ...
      */
      ...
      if (i_counter < to && !FLAGREADEDFILE1) {
        ...
        bloom_add(&bloom_bP[bloom_bP_index], rawvalue, BSGS_BUFFERXPOINTLENGTH);
          ...
          bloom_add(&bloom_bP[bloom_bP_index], rawvalue,
                    BSGS_BUFFERXPOINTLENGTH);
...
dst_address: a pointer to an unsigned char array where the generated binary
address will be stored. The function is designed to generate a binary address
for Ethereum using the given public key. It first extracts the x and y
coordinates of the public key as 32-byte arrays, and concatenates them to form a
64-byte array called bin_publickey. Then, it applies the KECCAK-256 hashing
algorithm to bin_publickey to generate the binary address, which is stored in
dst_address. */
...
The function is designed to convert the raw data using a lookup table
(Ccoinbuffer) and store the result in the buffer.
*/
void set_minikey(char *buffer, char *rawbuffer, int length) {
...
buffer: a pointer to a char array where the minikey will be stored.
rawbuffer: a pointer to a char array that contains the raw data.
index: an integer representing the index of the raw data array to be
incremented. The function is designed to increment the value at the specified
index in the raw data array, and update the corresponding value in the buffer
using a lookup table (Ccoinbuffer). If the value at the specified index exceeds
57, it is reset to 0x00 and the function recursively calls itself to increment
the value at the previous index, unless the index is already 0, in which case
the function returns false. The function returns true otherwise.
*/
...
#define BUFFMINIKEY(buff, src)                                                 \
  (buff)[0] = (uint32_t)src[0] << 24 | (uint32_t)src[1] << 16 |                \
              (uint32_t)src[2] << 8 | (uint32_t)src[3];                        \
  (buff)[1] = (uint32_t)src[4] << 24 | (uint32_t)src[5] << 16 |                \
              (uint32_t)src[6] << 8 | (uint32_t)src[7];                        \
  (buff)[2] = (uint32_t)src[8] << 24 | (uint32_t)src[9] << 16 |                \
              (uint32_t)src[10] << 8 | (uint32_t)src[11];                      \
  (buff)[3] = (uint32_t)src[12] << 24 | (uint32_t)src[13] << 16 |              \
              (uint32_t)src[14] << 8 | (uint32_t)src[15];                      \
  (buff)[4] = (uint32_t)src[16] << 24 | (uint32_t)src[17] << 16 |              \
...
#define BUFFMINIKEYCHECK(buff, src)                                            \
  ...
  (buff)[4] = (uint32_t)src[16] << 24 | (uint32_t)src[17] << 16 |              \
              (uint32_t)src[18] << 8 | (uint32_t)src[19];                      \
  (buff)[5] = (uint32_t)src[20] << 24 | (uint32_t)src[21] << 16 |              \
              (uint32_t)src[22] << 8 | 0x80;                                   \
  (buff)[6] = 0;                                                               \
  (buff)[7] = 0;                                                               \
  (buff)[8] = 0;                                                               \
  (buff)[9] = 0;                                                               \
...
void menu() {
  ...
  printf("-z value    Bloom size multiplier, only address,rmd160,vanity, "
         "xpoint, value >= 1\n");
  printf("\nExample:\n\n");
  printf("./keyhunt -m rmd160 -f tests/unsolvedpuzzles.rmd -b 66 -l compress "
         "-R -q -t 8\n\n");
...
bool vanityrmdmatch(unsigned char *rmdhash) {
  ...
  result =
      bloom_check(vanity_bloom, rmdhash, vanity_rmd_minimun_bytes_check_length);
  switch (result) {
  case -1:
...
int addvanity(char *target) {
  ...
  if (targetsize >= 30) {
    printf("[*] Exportando hits encontrados...\n");
  ...
  j = 0;
  vanity_address_targets = (char **)realloc(
      vanity_address_targets, (vanity_rmd_targets + 1) * sizeof(char *));
  vanity_address_targets[vanity_rmd_targets] = NULL;
  checkpointer((void *)vanity_address_targets, __FILE__, "realloc",
               "vanity_address_targets", __LINE__ - 1);
  vanity_rmd_limits =
      (int *)realloc(vanity_rmd_limits, (vanity_rmd_targets + 1) * sizeof(int));
  vanity_rmd_limits[vanity_rmd_targets] = 0;
  checkpointer((void *)vanity_rmd_limits, __FILE__, "realloc",
               "vanity_rmd_limits", __LINE__ - 1);
  vanity_rmd_limit_values_A =
      (uint8_t ***)realloc(vanity_rmd_limit_values_A,
                           (vanity_rmd_targets + 1) * sizeof(unsigned char *));
  checkpointer((void *)vanity_rmd_limit_values_A, __FILE__, "realloc",
               "vanity_rmd_limit_values_A", __LINE__ - 1);
  vanity_rmd_limit_values_A[vanity_rmd_targets] = NULL;
  vanity_rmd_limit_values_B =
      (uint8_t ***)realloc(vanity_rmd_limit_values_B,
                           (vanity_rmd_targets + 1) * sizeof(unsigned char *));
  checkpointer((void *)vanity_rmd_limit_values_B, __FILE__, "realloc",
               "vanity_rmd_limit_values_B", __LINE__ - 1);
  ...
  do {
    ...
    if (raw_value_length == 25) {
      ...
      vanity_rmd_limit_values_A[vanity_rmd_targets] =
          (uint8_t **)realloc(vanity_rmd_limit_values_A[vanity_rmd_targets],
                              (j + 1) * sizeof(unsigned char *));
      checkpointer((void *)vanity_rmd_limit_values_A[vanity_rmd_targets],
                   __FILE__, "realloc", "vanity_rmd_limit_values_A",
                   __LINE__ - 1);
      vanity_rmd_limit_values_A[vanity_rmd_targets][j] =
          (uint8_t *)calloc(20, 1);
      checkpointer((void *)vanity_rmd_limit_values_A[vanity_rmd_targets][j],
                   __FILE__, "realloc", "vanity_rmd_limit_values_A",
                   __LINE__ - 1);

      memcpy(vanity_rmd_limit_values_A[vanity_rmd_targets][j], raw_value_A + 1,
             20);
  ...
  do {
    ...
    if (raw_value_length == 25) {
      ...
      vanity_rmd_limit_values_B[vanity_rmd_targets] =
          (uint8_t **)realloc(vanity_rmd_limit_values_B[vanity_rmd_targets],
                              (j + 1) * sizeof(unsigned char *));
      checkpointer((void *)vanity_rmd_limit_values_B[vanity_rmd_targets],
                   __FILE__, "realloc", "vanity_rmd_limit_values_B",
                   __LINE__ - 1);
      checkpointer((void *)vanity_rmd_limit_values_B[vanity_rmd_targets],
                   __FILE__, "realloc", "vanity_rmd_limit_values_B",
                   __LINE__ - 1);
      vanity_rmd_limit_values_B[vanity_rmd_targets][j] =
          (uint8_t *)calloc(20, 1);
      checkpointer((void *)vanity_rmd_limit_values_B[vanity_rmd_targets][j],
                   __FILE__, "calloc", "vanity_rmd_limit_values_B",
                   __LINE__ - 1);
      memcpy(vanity_rmd_limit_values_B[vanity_rmd_targets][j], raw_value_B + 1,
             20);
  ...
  if (values_A_size >= 1 && values_B_size >= 1) {
    ...
    for (j = 0; j < r; j++) {
      minimun_bytes = minimum_same_bytes(
          vanity_rmd_limit_values_A[vanity_rmd_targets][j],
          vanity_rmd_limit_values_B[vanity_rmd_targets][j], 20);
      if (minimun_bytes < vanity_rmd_minimun_bytes_check_length) {
        vanity_rmd_minimun_bytes_check_length = minimun_bytes;
      }
    }
    vanity_address_targets[vanity_rmd_targets] =
        (char *)calloc(targetsize + 1, sizeof(char));
    checkpointer((void *)vanity_address_targets[vanity_rmd_targets], __FILE__,
                 "calloc", "vanity_address_targets", __LINE__ - 1);
    memcpy(vanity_address_targets[vanity_rmd_targets], target,
           targetsize + 1); // +1 to copy the null character
...
bool processOneVanity() {
  ...
  for (i = 0; i < vanity_rmd_targets; i++) {
    for (k = 0; k < vanity_rmd_limits[i]; k++) {
      bloom_add(vanity_bloom, vanity_rmd_limit_values_A[i][k],
                vanity_rmd_minimun_bytes_check_length);
    }
  }
  return true;
      ...
      if (hextemp == aux) {
        ...
        if (len > 0 && len < 36) {
          if (isValidBase58String(aux)) {
            ...
            fprintf(stderr,
                    "[E] the string \"%s\" is not valid Base58, omiting it\n",
                    aux);
          }
        }
      }
...
bool readFileAddress(char *fileName) {
  FILE *fileDescriptor;
  char fileBloomName[30]; /* Actually it is Bloom and Table but just to keep the
                             variable name short*/
  ...
  uint64_t dataSize;
  /*
          if the FLAGSAVEREADFILE is Set to 1 we need to the checksum and check
     if we have that information already saved
  */
  if (FLAGSAVEREADFILE) { /* if the flag is set to REAd and SAVE the file firs
                             we need to check it the file exist*/
    if (!sha256_file((const char *)fileName, checksum)) {
      fprintf(stderr, "[E] sha256_file error line %i\n", __LINE__ - 1);
      return false;
    }
    tohex_dst((char *)checksum, 4,
              (char *)hexPrefix); // we save the prefix (last fourt bytes)
                                  // hexadecimal value
    ...
    if (fileDescriptor != NULL) {
      ...
      */
      ...
      printf("[+] Allocating memory for %" PRIu64 " elements: %.2f MB\n", N,
             (double)(((double)sizeof(struct address_value) * N) /
                      (double)1048576));
...
bool forceReadFileAddress(char *fileName) {
  ...
  printf("[+] Allocating memory for %" PRIu64 " elements: %.2f MB\n",
         numberItems,
         (double)(((double)sizeof(struct address_value) * numberItems) /
                  (double)1048576));
  addressTable = (struct address_value *)malloc(sizeof(struct address_value) *
                                                numberItems);
  checkpointer((void *)addressTable, __FILE__, "malloc", "addressTable",
               __LINE__ - 1);
    ...
    if (r > 0 && r <= 40) {
      if (r < 40 && isValidBase58String(aux)) { // Address
        ...
        if (raw_value_length == 25) {
          ...
          memcpy(addressTable[i].value, rawvalue + 1,
                 sizeof(struct address_value));
          i++;
          validAddress = true;
        }
      }
...
bool initBloomFilter(struct bloom *bloom_arg, uint64_t items_bloom) {
  ...
  if (items_bloom <= 10000) {
    ...
    if (bloom_init2(bloom_arg, FLAGBLOOMMULTIPLIER * items_bloom, 0.000001) ==
        1) {
...
void writeFileIfNeeded(const char *fileName) {
  // printf("[D] FLAGSAVEREADFILE %i, FLAGREADEDFILE1
