...
int main(int argc, char **argv) {
    ...
    // Registra a função base58 -> sha256
    ...
    size_t rsize = 0;        // para fread/fwrite
	...
	M3	5497558139
		*/
		...
		if(FLAGSAVEREADFILE)	{
			size_t readed = 0; // <--- declare aqui para uso nos fread
			...
			if(fd_aux3 != NULL)	{
				...
				fflush(stdout);
				size_t rsize = fread(bPtable,bytes,1,fd_aux3);
				if(rsize != 1)	{
					fprintf(stderr,"[E] Error reading the file %s\n",buffer_bloom_file);
					exit(EXIT_FAILURE);
				}
...
size_t readed = 0; // <-- declarar antes de usar fwrite
...
if(!FLAGREADEDFILE3)	{
	...
	if(fd_aux3 != NULL)	{
		...
		fflush(stdout);
		size_t rsize = fwrite(bPtable,bytes,1,fd_aux3);
		if(rsize != 1)	{
			fprintf(stderr,"[E] Error writing the file %s\n",buffer_bloom_file);
			exit(EXIT_FAILURE);
		}
